        ...
        private void DrawContextKeyItemsEditor(SerializedProperty property)
        {
            ...
            var text =
                SyntaxHighlighter.ColorizeSyntax(
                 "\n" +
                 "// Инициализация:\r\n" +
                 "1. Возьмите в папке пакета во вкладке UI префаб кросс-промо и добавьте в Naninovel UI.\r\n" +
                 "2. Зайдите в папку DemoTitleButton и поместите префаб кнопки открытия кросс-промо в ваше главное меню.\r\n" +
                 "3. В инспекторе Naninovel укажите ссылку на таблицу (проверьте корректность ссылки, она должна заканчиваться на .csv).\r\n" +
                 "4. Установите статические данные в поля adultStatic для каждого элемента.\r\n" +
                 "5. Установите в поле achievementNaniCommand команду для открытия ачивки при сборе всех слотов.\r\n\n" +
                 ...
                 "• В папке DemoCGSlot лежит пример слота для галереи.\n" +
                 "• Также добавлены Unity события для различных действий, чтобы можно было подключать частицы и т.д.\r\n\r\n" +
                 "<color=#fff9c7>P.S. В LeaderBoardCoroutines уже есть реализация инвока steam_id лидерборда (первого захода в игру).</color>\n\n" +
